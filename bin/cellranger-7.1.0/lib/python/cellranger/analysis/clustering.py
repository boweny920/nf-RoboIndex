...
from typing import TYPE_CHECKING, ClassVar, NamedTuple, Protocol, Union
...
def parse_clustering_key(clustering_key: str):
    ...
    if clustering_key in (CLUSTER_TYPE_GRAPHCLUST, CLUSTER_TYPE_ATAC_GRAPHCLUST):
...
def create_clustering(
    clusters: Union[np.ndarray[int, np.dtype[np.int64]], Sequence[int]],
    num_clusters: int,
    cluster_score: float,
    clustering_type: Union[str, bytes],
