...
class CellsPerFeature(MutableMapping):
    ...
    @staticmethod
    def load_from_file(filename: Union[str, bytes]) -> CellsPerFeature:
    ...
    @staticmethod
    def from_dictionary(data: dict[Union[str, bytes], list[Union[str, bytes]]]) -> CellsPerFeature:
...
class FeatureAssignmentsMatrix:
    ...
    def get_features_per_cell_table(self, sep=b"|"):
        ...
        for row in self.df.T.itertuples():
            ...
            bc_data = np.ravel(
                self.matrix.m.getcol(cell_index).todense()
...
def validate_force_sample_barcodes(
    ...
    """If the user overrides the sample deplex using a CSV file, we need to verify the barcodes in the CSV match.

    the results from cell calling, or the behavior is undefined (and as of 3/14/2022 leads to a downstream error).
    Args:
