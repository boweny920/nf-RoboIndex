...
def decode_ascii_xml(x: Union[str, bytes]) -> str:
...
def encode_ascii_xml(x: Union[str, bytes]):
...
def encode_ascii_xml_array(
    data: Union[np.ndarray, Iterable[Union[str, bytes]]]
...
_mlastConverted: Optional[Union[str, bytes]] = None
_mlastValue: Optional[str] = None
...
def read_hdf5_string_dataset(dataset: h5py.Dataset, memoize: bool = False) -> list[str]:
    ...
    if memoize:
        ...
        def decoder(x: Union[str, bytes]) -> str:
...
def set_hdf5_attr(
    dataset, name: Union[str, bytes], value: Union[str, bytes, Iterable[Union[str, bytes]]]
):
...
def combine_h5s_into_one(outfile: Union[str, bytes], files_to_combine: Iterable[Union[str, bytes]]):
