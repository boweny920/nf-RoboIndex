...
class MoleculeCounter:
    ...
    def __init__(self):
        ...
        self.library_info: Optional[list[dict[str, Any]]] = None
        self.feature_reference: Optional[FeatureReference] = None

    def get_gb_barcode_whitelist(self):
        return barcode_whitelist_from_metrics(self.get_all_metrics())

    def get_turing_slide_name(self):
    ...
    def get_molecule_info_type(self) -> bool:
        metrics = self.get_all_metrics()
        return metrics.get(MOLECULE_INFO_TYPE_METRIC, MOLECULE_INFO_TYPE_COUNT)
    ...
    def open(
        ...
        filename,
        mode: str,
        feature_ref: Optional[FeatureReference] = None,
        barcodes=None,
        library_info=None,
    ...
    def is_spatial_data(self) -> bool:
        ...
        return (
            ...
            or self.get_turing_slide_name() is not None
        )
    ...
    def get_column_with_indices(
        ...
        if idxs.shape == (nmol,) and idxs.dtype == bool:
            ...
            # index value array
            raise RuntimeError("index value arrays not yet supported")
        return vals
    ...
    @staticmethod
    def concatenate(out_filename, in_filenames, metrics: Optional[dict] = None) -> None:
    ...
    def get_raw_read_pairs_per_library(self, all_metrics: Optional[dict] = None) -> list[int]:
        ...
        try:
            ...
            raise RuntimeError("Missing metrics in molecule counter")

    def get_read_pairs_in_filtered_barcodes_per_library(
        self, all_metrics: Optional[dict] = None
    ) -> list[int]:
