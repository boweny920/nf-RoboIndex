...
def to_numeric_safe(
    x: Union[float, int, tuple, np.ndarray, pd.Series], downcast: Optional[str] = "unsigned"
):
...
def _get_idx_mol(
    mc: MoleculeCounter,
    filter_library_idx: Union[int, list[int], None] = None,
    filter_feature_idx: Union[int, list[int], None] = None,
    filter_barcode_idx: Union[int, list[int], None] = None,
) -> np.ndarray[int, np.dtype[np.bool_]]:
    if filter_library_idx is None:
        idx_mol = np.ones(mc.nrows(), dtype=np.bool_)
    else:
...
def _get_is_cell(
    ...
    with_cell_call: bool,
) -> Optional[np.ndarray[int, np.dtype[np.bool_]]]:
    ...
    if exclude_cells and exclude_noncells:
        ...
        is_cell = np.zeros(np.sum(idx_mol), dtype=np.bool_)
...
def _mol_info_df_from_h5(  # pylint: disable=too-many-arguments
    ...
    with_cell_call: bool,
    with_umi_type: bool,
    filter_library_idx: Union[int, list[int], None],
    filter_feature_idx: Union[int, list[int], None],
    filter_barcode_idx: Union[int, list[int], None],
...
def summarize_by_barcode(
    mol_info_df: pd.DataFrame, feature_idx_subsets: Optional[dict[str, list[int]]] = None
):
...
def collapse_feature_counts(
    mc: MoleculeCounter,
    filter_library_idx: Optional[list[int]] = None,
...
def _barcode_is_cell(
    ...
    filter_library_idx: Optional[list[int]],
    num_barcodes,
):
