...
class ReferenceBuilder(NewGtfParser):
    def __init__(
            ...
            for j, g2 in enumerate(self.genomes):
                if i != j and g1.startswith(g2):
                    raise GexReferenceError(
                        "Supplied genome name '{g2}' is a prefix of genome name '{g1}'".format(
                            g1=g1, g2=g2
                        )
                    )
        ...
        self.gtf_path: Optional[str] = None
        self.genome_prefixes: Optional[list[str]] = None
    ...
    def process_fasta(self):
        ...
        samtools = os.path.join(os.path.dirname(sys.executable), "samtools")
    ...
    def process_gtf(
        self, contig_lengths: Optional[dict[str, int]] = None, no_transcript_fail: bool = False
    ):
        """Parse and write GTF and compute gtf pickle."""
        assert self.fasta_path and os.path.exists(
            self.fasta_path
        ), "FASTA file not present in fasta/genome.fa"
    ...
    def build_gex_reference(self, out_dir_exists: bool = False):
        ...
        try:
            ...
            print(
                ...
                "Error was from running command '{}'\n{}\n\n"
                "Check stdout and stderr for more information.\n".format(err.cmd[0], err)
            )
            sys.exit(1)
    ...
    def write_genome_fasta(self, out_fasta_fn: Union[os.PathLike, str, bytes]):
    ...
    def write_genome_gtf(
        self,
        out_gtf_fn: Union[os.PathLike, str, bytes],
