...
SUBSAMPLE_FIXED_DEPTHS = [5000, 10000, 20000, 30000, 50000]
...
def compute_target_depths(
    ...
    distinct_targets = np.unique(
        np.linspace(start=0, stop=max_target, num=num_targets + 1, dtype=np.int_)
    )
...
def _subsampling_for_depth(
    ...
    max_computed_depth: Optional[int],
...
def make_subsamplings(
    ...
    target_depths: np.ndarray[
        ...
        [computed_depths, fixed_depths], dtype=np.int_
    )
    target_depths.sort()
...
def run_subsampling(
    ...
    filtered_barcodes_csv,
    feature_indices: Optional[Collection[int]],
    chunk_start,
    chunk_len,
) -> SubsampleDataDict:
    ...
    with cr_mc.MoleculeCounter.open(molecule_info_h5, "r") as mc:
        ...
        lib_type_genome_any_reads: np.ndarray[tuple[int, int], np.dtype[np.bool_]] = np.zeros(
            (len(lib_types), len(genomes)), dtype=np.bool_
        )
