    ...
    for filename in required_files:
        ...
        if not os.path.isfile(p):
            raise PreflightException(
                "Your reference does not contain the expected files, including %s, or they are not readable. Please check your reference folder on %s."
                % (p, hostname)
            )
...
def check_inner_enrichment_primers(primers_file, reference_path):
    ...
    if not os.path.isfile(primers_file):
        raise PreflightException(
            "The file specifying inner enrichment primers (%s), does not exists or is not readable. Please check your path on %s."
    ...
    with open(primers_file) as f:
        for i, line in enumerate(f.readlines()):
            ...
            if len(seq) == 0:
                raise PreflightException(
                    "Line number %s in the inner enrichment primers file (%s) is empty. You should specify a newline separated list of primers."
                    % (i + 1, primers_file)
                )
            for j, base in enumerate(seq):
                if base not in {"A", "C", "G", "T"}:
                    raise PreflightException(
                        "Inner enrichment primers file (%s) contain non ACGT characters, which are not supported (Found %s in line %s, character %s). You should specify a newline separated list of primers."
    ...
    if reference_path:
        ...
        if invalid:
            ...
            raise PreflightException(
                "None of the C-REGIONs in the reference %s is targeted by the following inner enrichment primer(s): %s."
                % (reference_path, ensure_str(invalid_str))
            )
