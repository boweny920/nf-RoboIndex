...
import os
import shutil
from typing import AnyStr, NamedTuple, Optional, TypedDict, Union
...
class VdjAnnotationFeature(NamedTuple):
    ...
    gene_name: bytes
    region_type: bytes
    chain_type: Optional[bytes]
    chain: Optional[bytes]
    isotype: Optional[bytes]
    allele_name: Optional[bytes]
...
def standardize_ensembl_gene_name(gene_name: str) -> Optional[str]:
...
def build_reference_fasta_from_ensembl(
    ...
    for gtf in gtf_paths:
        ...
        for line_no, entry in enumerate(get_gtf_iter(gtf)):
            ...
            if vdj_feature is None:
                print(
                    "Warning: Transcript %s has biotype %s. Could not infer VDJ gene type. Skipping."
    ...
    for (transcript_id, region_type), regions in transcripts.items():
        if not all(r.chrom == regions[0].chrom for r in regions):
            ...
            print(
                "Warning: Transcript %s spans multiple contigs: %s. Skipping."
        ...
        if chrom not in genome_fasta:
            print(
                'Warning: Transcript %s is on contig "%s" which is not in the provided reference fasta. Skipping.'
                % (transcript_id, chrom)
            )
            continue
        ...
        def _bytes_or_none(maybe_str: Optional[AnyStr]) -> Optional[bytes]:
        ...
        if feat_key in seen_features:
            print(
                "Warning: Skipping duplicate entry for %s (%s, %s)."
...
def build_reference_fasta_from_fasta(
    ...
    seen_features: set[
        tuple[bytes, bytes, bytes, Optional[bytes], Optional[bytes], Optional[bytes]]
    ] = set()
    ...
    with open(fasta_path, "rb") as f:
        for header, sequence in cr_utils.get_fasta_iter(f):
            ...
            if feat.chain_type not in chain_types.VDJ_CHAIN_TYPES:
                print(
                    "Warning: Unknown chain type for: %s. Expected name to be in %s. Skipping."
                    % (
                        str(
                            (
...
def make_display_name(gene_name: Optional[str], allele_name: Optional[str]) -> Optional[str]:
