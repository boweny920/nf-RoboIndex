...
def make_treemap_help(is_antibody):
    singular = "Antibody" if is_antibody else "Antigen"
    cell_calls = "cell barcodes" if is_antibody else "GEX cell barcodes"
    return {
        "helpText": f"Relative composition of {singular.lower()} counts for features with at least {MIN_ANTIBODY_UMI} UMI.\n"
        f"Box size represents fraction of total UMIs from {cell_calls} that are derived from this {singular.lower()}."
...
def make_antibody_treemap_plot(ab_matrix, lib_type, subset_features, is_spatial=True):
    ...
    hover_antibody_labs = ["<b>Feature ID (% Total UMI):</b><br>" + ab for ab in antibody_label]
    ...
    treemap_plot_data = {"plot": treemap_plot, "help": make_treemap_help(is_antibody)}
