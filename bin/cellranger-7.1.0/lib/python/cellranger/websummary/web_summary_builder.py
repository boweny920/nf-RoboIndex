...
def _build_summary_tab_common(
    ...
    # Targeted hybridcap with or without antibody and not aggr
    # (the aggr pipeline doesn't generate targeted metrics)
    if (
        sample_data.is_targeted()
        and sample_data.targeting_method == TARGETING_METHOD_HC
        and not isinstance(sample_properties, wsp.AggrCountSampleProperties)
    ):
...
def _build_analysis_tab_common(metadata, sample_data, species_list, sample_properties, ws_data):
...
def build_web_summary_data_common(
    ...
    if FILTERED_BCS_TRANSCRIPTOME_UNION in sample_data.summary:
        ...
        _build_analysis_tab_common(metadata, sample_data, species_list, sample_properties, wsd)
