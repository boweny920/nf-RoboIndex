...
READ_NAME_REGEX = r"^[a-zA-Z0-9\-]+:[a-z0-9\-]+:([a-zA-Z0-9\-]+):([0-9]):([0-9])([0-9])([0-9]+):([0-9]+):([0-9]+)$"

NUM_READS_TO_ESTIMATE_TILE_EXTENTS = 2000


def extract_read_position(read) -> Optional[ReadLocation]:
...
class DeclaredFlowcellLayout:
    ...
    NAMING_CONVENTION_TILE_LENGTHS = {"FiveDigit": 5, "FourDigit": 4}
    ...
    def __init__(
        ...
        *args,
        ...
        section_per_lane: Optional[int] = None,
        lane_per_section: Optional[int] = None,
    ...
    @classmethod
    def from_flowcell_layout_node(cls, node: etree.Element):
        """Construct a PatternedFlowcellLayout object from RunInfo.xml.

        Reads the FlowcellLayout node on RunInfo.xml.
        """

        def intAttrOrNone(key: str):
            return int(node.attrib.get(key)) if node.attrib.get(key) is not None else None
...
class LaneLayout:
    ...
    def has_diffusion_duplicates(self, diffusion_radius: float):
