...
def validate_csv(csv_file, entry_type, entry_colname):
    if not os.path.exists(csv_file):
        ...
        martian.exit(
            "Specified %s file is not readable, please check file permissions: %s"
            % (entry_type, csv_file)
        )
    with open(csv_file) as f:
        ...
        if header[0] != entry_colname:
            martian.exit(
                "First line of %s file must be a header line, with '%s' as the first column."
                % (entry_type, entry_colname)
            )
