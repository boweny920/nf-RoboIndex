...
def join(args, outs, chunk_defs, chunk_outs):
    ...
    pca = load_pca_from_h5(args.pca_h5)[0].transformed_pca_matrix
    assert pca.shape[0] == len(use_bcs)
    pca = pca[use_bcs, :]
    ...
    while True:
        ...
        for step in range(hc.shape[0]):
            if hc[step, 0] <= max_label and hc[step, 1] <= max_label:
                ...
                print(f"Comparing clusters ({1 + leaf0},{1 + leaf1})")
                submat = expr_mat[:, np.concatenate((group0, group1))]

                print(f"\tComputing params on {submat.shape} matrix")
                params = compute_sseq_params(submat)

                print(f"\tRunning DE on {len(group0)} vs {len(group1)} cells")
                ...
                if n_de_genes == 0:
                    print(
                        "\tFound {} DE genes. Merging clusters ({},{})".format(
