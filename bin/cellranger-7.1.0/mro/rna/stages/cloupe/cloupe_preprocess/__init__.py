...
def do_not_make_cloupe(args):
    ...
    if args.product_type == cr_constants.SPATIAL_PRODUCT_TYPE:
        ...
        if args.aggregation_csv is None:
            # Case4
            if not bc_utils.is_whitelist_spatial(args.barcode_whitelist):
...
def split(args):
    ...
    # See https://github.com/10XDev/cellranger/pull/1196 for a discussion of
    # Where this memory estimate is derived from.  Note that analysis did not account for imaging data.

    mem_gb = cr_matrix.CountMatrix.get_mem_gb_crconverter_estimate_from_h5(
        args.filtered_gene_bc_matrices_h5
    )

    chunks = [{"__mem_gb": max(mem_gb, h5_constants.MIN_MEM_GB, 6), "__threads": 2}]
...
def main(args, outs):
    ...
    image_page_names = get_image_page_names(args.loupe_alignment_file)
    if len(image_page_names):
        call.extend(["--spatial-image-page-names", (",").join(image_page_names)])
