...
from cellranger.websummary.analysis_tab_aux import cmo_tags_on_tsne_from_path
from cellranger.websummary.analysis_tab_core import umi_on_tsne_from_path
...
def main(args, outs):
    ...
    if (
        not args.multiplexing_is_rtl
        and args.analysis is not None
        and cells_per_tag is not None
        and args.non_singlet_barcodes is not None
    ):
        cmo_umi_tsne_plot = umi_on_tsne_from_path(
            args.analysis, library_type=rna_library.MULTIPLEXING_LIBRARY_TYPE
        )
        ...
        cmo_tags_tsne_plot = cmo_tags_on_tsne_from_path(
            args.analysis,
            cells_per_tag,
            non_singlet_barcodes,
        )
        cmo_tags_tsne_plot["layout"]["title"] = None

        cmo_tsne_plot = {
            "cmo_umi_tsne_plot": cmo_umi_tsne_plot,
            "cmo_tags_tsne_plot": cmo_tags_tsne_plot,
        }

        with open(outs.cmo_tsne_plot, "w") as out:
            json.dump(
                tk_safe_json.json_sanitize(cmo_tsne_plot),
                out,
                sort_keys=True,
                indent=4,
