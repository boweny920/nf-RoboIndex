...
def main(args, outs):
    ...
    for sample, molecule_info in sample_molecule_infos.items():
        ...
        sample_files["filtered_matrix_h5"] = hard_link(
            sample_matrices[sample]["filtered_matrix_h5"]
        )
        sample_files["raw_matrix_h5"] = hard_link(sample_matrices[sample]["raw_matrix_h5"])
        sample_files["filtered_barcodes"] = hard_link(sample_matrices[sample]["filtered_barcodes"])
        ...
        for prefix in ["filtered_", "raw_"]:
            ...
            matrix_mex_dir = os.path.basename(sample_matrices[sample][key])
            sample_files[key] = martian.make_path(matrix_mex_dir).decode("utf8")
            os.makedirs(sample_files[key])
            hard_link(
                os.path.join(sample_matrices[sample][key], "barcodes.tsv.gz"),
                relative_path=os.path.join(matrix_mex_dir, "barcodes.tsv.gz"),
            )
            hard_link(
                os.path.join(sample_matrices[sample][key], "features.tsv.gz"),
                relative_path=os.path.join(matrix_mex_dir, "features.tsv.gz"),
            )
            hard_link(
                os.path.join(sample_matrices[sample][key], "matrix.mtx.gz"),
        ...
        sample_files["metrics_summary"] = hard_link(
            sample_metrics[sample]["summary"],
            relative_path=sample + "_" + os.path.basename(sample_metrics[sample]["summary"]),
        )
        sample_files["per_barcode_metrics"] = hard_link(
            sample_metrics[sample]["per_barcode_metrics"],
            relative_path=sample
            + "_"
            + os.path.basename(sample_metrics[sample]["per_barcode_metrics"]),
        )

        sample_outs[sample] = sample_files
