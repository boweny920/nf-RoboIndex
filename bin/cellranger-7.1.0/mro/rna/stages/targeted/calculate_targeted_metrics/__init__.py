...
"""Iterate over the input BAM file and calculate on-target and off-target on a per-read basis.
...
def split(args):
    with cr_mc.MoleculeCounter.open(args.molecule_info, "r") as molecule_info:
        mol_info_mem_gb = molecule_info.estimate_mem_gb(molecule_info.nrows(), scale=4.0)
    return {"chunks": [], "join": {"__mem_gb": mol_info_mem_gb}}
...
def get_enrichment_metrics(
    ...
    is_spatial = cr_mc.MoleculeCounter.open(molecule_info, "r").is_spatial_data()
...
def get_mean_per_gene_rpu_metrics(molecule_info, pfm):
    ...
    for targeting in [True, False]:
        ...
        for in_cells in [True, False]:
            ...
            if is_spatial:
                per_gene_rpu_metrics[
                    "spatial_mean_reads_per_umi_per_gene{}_{}".format(
                        in_cells_suffix, target_suffix
                    )
                ] = mean_rpu
...
def join(args, outs, chunk_defs, chunk_outs):
    ...
    # get RPU metrics
    targeted_metrics.update(get_mean_per_gene_rpu_metrics(args.molecule_info, feature_summary_df))
    ...
    feature_summary_df, enrichment_calc_metrics = get_enrichment_metrics(
        args.molecule_info, feature_summary_df, disable_rpu_enrichments, method=BOTH_TIED
    )
