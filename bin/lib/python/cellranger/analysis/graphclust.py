...
def compute_snn_matrix(
    nn: sp_sparse.spmatrix, k_nearest: Union[float, int]
) -> sp_sparse.coo_matrix:
...
def load_louvain_results(
    num_barcodes: int, use_bcs: np.ndarray[int, np.dtype[np.uintp]], louvain_out: Union[str, bytes]
...
def save_neighbor_index(index: sk_neighbors.BallTree, filename: Union[str, bytes]):
    with open(filename, "wb") as f:
        cPickle.dump(index, f, cPickle.HIGHEST_PROTOCOL)


def load_neighbor_index(filename: Union[str, bytes]) -> sk_neighbors.BallTree:
