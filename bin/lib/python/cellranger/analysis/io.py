...
from typing import NamedTuple, TypeVar, Union, overload
...
def _string_or_num(value: Union[str, bytes, int, float]):
...
def _string_or_num_list(
    value: Union[str, bytes, int, float, Iterable[Union[str, bytes, int, float]]]
...
def save_matrix_csv(
    ...
    arr: Iterable[Union[str, bytes, int, float, Iterable[Union[str, bytes, int, float]]]],
    header: Iterable[str],
    prefixes: Iterable[Union[str, bytes, int, float, Iterable[Union[str, bytes, int, float]]]],
):
...
_T1 = TypeVar("_T1", bound=NamedTuple)
...
def load_dimension_reduction_from_h5(filename: str, group_name: str, ntuple: type[_T1]):
    ...
    with tables.open_file(filename, "r") as f:
        ...
        # otherwise for PCA, we potentially generate both GEX- and Ab-based projections
        gex_pca, ab_pca = None, None
        ...
        if gex_pca and ab_pca:
            ...
            raise ValueError("Expected at least one of gene_expression or antibody_capture")
