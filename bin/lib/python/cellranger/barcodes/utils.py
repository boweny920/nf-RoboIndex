...
@overload
def load_barcode_tsv(filename: Union[str, bytes], as_set: Literal[False] = False) -> list[bytes]:
    ...


@overload
def load_barcode_tsv(filename: Union[str, bytes], as_set: Literal[True] = ...) -> set[bytes]:
    ...


@overload
def load_barcode_tsv(
    filename: Union[str, bytes], as_set: bool = False
) -> Union[set[bytes], list[bytes]]:
    ...
...
@overload
def load_barcode_whitelist(filename: str, as_set: bool) -> Union[set[bytes], list[bytes]]:
    ...
...
def load_barcode_translate_map(bc_whitelist: Optional[str]):
...
def get_mem_gb_request_from_barcode_whitelist(
    barcode_whitelist_fn: Optional[str], gem_groups=None, use_min=True, double=False
):
