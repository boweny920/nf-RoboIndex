...
class LogPerf:
    ...
    def __enter__(self):
        ...
        print(
            "%s\tself_premaxrss_mb\t%0.1f"
            % (self.note, resource.getrusage(resource.RUSAGE_SELF)[2] / 1e3)
        )
        print(
            "%s\tchildren_premaxrss_mb\t%0.1f"
            % (self.note, resource.getrusage(resource.RUSAGE_SELF)[2] / 1e3)
        )
        sys.stdout.flush()
    ...
    def __exit__(self, e_type, e_value, e_trace):
        print(
            "%s\tself_postmaxrss_mb\t%0.1f"
            % (self.note, resource.getrusage(resource.RUSAGE_SELF)[2] / 1e3)
        )
        print(
            "%s\tchildren_postmaxrss_mb\t%0.1f"
            % (self.note, resource.getrusage(resource.RUSAGE_CHILDREN)[2] / 1e3)
        )
