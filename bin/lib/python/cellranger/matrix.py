...
class CountMatrixView:
    ...
    def count_ge(
        self, axis: Optional[int], threshold: int
    ) -> Union[np.uint64, np.ndarray[int, np.dtype[np.uint64]]]:
    ...
    def select_barcodes(self, indices: Union[list[int], np.ndarray]):
        ...
        view = self._copy()
        mask = np.bincount(indices, minlength=len(view.bc_mask)).astype(np.bool_)
        mask[mask > 1] = 1
        view.bc_mask &= mask
        return view
    ...
    def select_features(self, indices: Union[list[int], np.ndarray]) -> CountMatrixView:
        ...
        view = self._copy()
        mask = np.bincount(indices, minlength=len(view.feature_mask)).astype(np.bool_)
        mask[mask > 1] = 1
...
class CountMatrix:
    ...
    def save_h5_file(
        ...
        extra_attrs: Optional[
            Mapping[Union[str, bytes], Union[str, bytes, Iterable[Union[str, bytes]]]]
        ] = None,
        sw_version=None,
    ):
    ...
    def load_columns_from_file(cls, group: h5.Group, start: int, end: int) -> CountMatrix:
        """Load from an HDF5 group only a subset of the columns. Very similar to
        CountMatrix.load()
    ...
    @staticmethod
    def get_genomes_from_h5(filename: Union[str, bytes]) -> list[str]:
    ...
    def get_counts_per_barcode_for_genome(
        self, genome: str, feature_type: Optional[str] = None
    ...
    @staticmethod
    def get_format_version_from_h5(filename: Union[bytes, str]):
        with h5.File(ensure_binary(filename), "r") as f:
            return CountMatrix._get_format_version_from_handle(f)

    @staticmethod
    def load_h5_file(filename, col_start: Optional[int] = None, col_end: Optional[int] = None):
