...
# For parsing Fingerprint
UNTAGGED_KEY = "Untagged"
TAGGED_KEY = "Tagged"
GEM_WELL_KEY = "gem_well"
...
class CellMultiplexingType(Enum):
    ...
    def from_str(val):
        ...
        for data in CellMultiplexingType:
            if data.value == val:
                return data
        raise Exception(f"Could not parse CellMultiplexingType from {val}")
...
class FeatureType(Enum):
    ...
    # TODO(azarchs): re-enable after pylint 2.8 is released.
    ...
    def from_str(val):
        ...
        for data in FeatureType:
            if data.value == val:
                return data
        raise Exception(f"Could not parse feature type from {val}")
...
class VdjChainType(Enum):
    ...
    def from_str(val):
        ...
        for data in VdjChainType:
            if data.value == val:
                return data
        raise Exception(f"Could not parse vdj chain type from {val}")
...
class LibraryType(Enum):
    ...
    def from_str(val):
        ...
        for data in LibraryType:
            if data.value == val:
                return data
        raise Exception(f"Could not parse library type from {val}")
...
class LibraryFeatures:
    ...
    def validate(self):
        ...
        if self.gex is not None:
            assert self.vdj is None and self.feature_types is None
        elif self.vdj is not None:
            assert self.gex is None and self.feature_types is None
        elif self.feature_types is not None:
            assert self.gex is None and self.vdj is None
        else:
            raise Exception("All features are none")
    ...
    def library_type(self):
        ...
        raise Exception("Library type cannot be inferred in the absence of any features")

    def library_features(self):
        """FeatureType if any associated with this object."""
    ...
    def from_json_val(val):
        ...
        raise Exception(f"Could not parse {val} as LibraryFeatures")
...
class LibraryDef:
    ...
    def __init__(self, physical_library_id, gem_well, library_features, fastq_ids):
        self.physical_library_id = physical_library_id
        self.gem_well = gem_well
        self.library_features = library_features
        self.fastq_ids = fastq_ids

    def __eq__(self, other):
        return (
            self.physical_library_id == other.physical_library_id
            and self.gem_well == other.gem_well
            and self.library_features == other.library_features
            and self.fastq_ids == other.fastq_ids
        )

    def __hash__(self):
        return hash(
            (self.physical_library_id, self.gem_well, self.library_features, self.fastq_ids)
        )
...
class Fingerprint:
    ...
    def __init__(
        ...
        cell_multiplexing_type: Optional[CellMultiplexingType] = None,
    ):
        self.gem_well = gem_well
        self.tag_name = tag_name
        self.cell_multiplexing_type = cell_multiplexing_type

    def __eq__(self, other):
        return (
            self.gem_well == other.gem_well
            and self.tag_name == other.tag_name
            and self.cell_multiplexing_type == other.cell_multiplexing_type
        )

    def __hash__(self):
        return hash((self.gem_well, self.tag_name, self.cell_multiplexing_type))

    @staticmethod
    def from_json_val(val):
        """Create a Fingerprint object from a dictionary."""
        assert isinstance(val, dict)
        if UNTAGGED_KEY in val:
            return Fingerprint(val[UNTAGGED_KEY])
        elif TAGGED_KEY in val:
            inner = val[TAGGED_KEY]
            return Fingerprint(
                inner[0],
                tag_name=inner[1],
                cell_multiplexing_type=CellMultiplexingType.from_str(inner[2]),
            )
        else:
            raise Exception(f"Could not parse fingerprint from {val}")

    def is_multiplexed(self) -> bool:
        return self.tag_name is not None

    def get_cell_multiplexing_type(self) -> Optional[CellMultiplexingType]:
...
class CrMultiGraph:
    ...
    def to_graphviz_digraph(self) -> Optional[AGraph]:
        ...
        for i, samp in enumerate(self.samples):
            ...
            for j, fingerprint in enumerate(samp.fingerprints):
                ...
                if is_multiplexed:
                    ...
                    g_tags.add_node(tag_node_id, id=tag_node_id, label=fingerprint.tag_name)
