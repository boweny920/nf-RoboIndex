    ...
    for sample_def in sample_defs:
        ...
        if pipeline == cr_constants.PIPELINE_COUNT:
            ...
            if library_type not in (None, rna_library.VDJ_LIBRARY_TYPE):
                msg = (
                    "You declared a library with library_type = '%s'. For the vdj pipeline, the library_type field in sample_def must be missing or '%s'"
                    % (library_type, rna_library.VDJ_LIBRARY_TYPE)
                )
                raise PreflightException(msg)
...
def check_targeting_preflights(
    ...
    reference_path: str,
    feature_reference_path: Optional[str],
    *,
    parse_files: bool,
    ...
    try:
        ...
        for metric in required_library_metrics:
            if metric not in library_metrics.keys():
                raise PreflightException(
                    "The molecule info file %s is too old and is missing the metric %s. Please rerun."
                    % (mol_info_fn, metric)
                )
...
def check_target_features_same(target_sets):
    ...
    # TODO (maria.alexis): make this check the mol_info hash instead. should be able to just reuse
...
def check_sample_info(sample_def, reference_path, full_check, feature_ref_path=None):
    feature_ref = None
