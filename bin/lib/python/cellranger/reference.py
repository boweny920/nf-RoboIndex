...
class Interval(NamedTuple):
    chrom: Union[str, bytes]  # TODO: be consistent
    start: int
    end: int
    length: int
    strand: Optional[str]
...
class Gene(NamedTuple):
    id: Union[str, bytes]  # TODO: be consistent
    name: str
    length: Optional[int]
...
class NewGtfParser:
    ...
    def gtf_reader_iter(
        self,
        filename: Union[os.PathLike, str, bytes],
        ...
        with cr_io.open_maybe_gzip(filename, "r") as f:
            ...
            for i, row in enumerate(reader):
                ...
                if start > end:
                    raise GtfParseError(
                        filename,
                        "Invalid GTF annotation on line {}:\n"
                        "{}\n"
                        "Start position of feature = {} > End position of feature = {}".format(
                            i + 1, row, start, end
                        ),
                    )
                if contig_lengths:
                    ...
                    if end > max_len:
                        raise GtfParseError(
                            ...
                            "Invalid GTF annotation on line {}:\n"
                            "{}\n"
                            "End position of feature = {} > contig {} length = {}".format(
                                i + 1, row, end, row[0], max_len
                            ),
                        )
                    if start < 1:
                        raise GtfParseError(
                            ...
                            "Invalid GTF annotation on line {}:\n"
                            "{}\n"
                            "Start position of feature = {} < 1".format(i + 1, row, start),
                        )
                ...
                if annotation == "transcript":
                    ...
                    if no_transcript_fail and tid != transcript_row_id:
                        raise GtfParseError(
                            ...
                            "on line {line} specifies an 'exon' annotation for a "
                            "transcript {tid}, but there is no 'transcript' row in "
                            "the GTF for {tid} that immediately precedes it.".format(
                                row=row, line=i + 1, tid=tid
                            ),
                        )
                elif annotation == "gene":
                    # See https://10xgenomics.slack.com/archives/C46N4SAS3/p1646257072899589
    ...
    @staticmethod
    def validate_gene_id(filename, properties: dict[str, Union[str, int]], i, row):
    ...
    def load_gtf(
        self, in_gtf_fn: Union[os.PathLike, str, bytes], fasta_parser: Optional[FastaParser] = None
    ...
    def get_properties_dict(
        ...
        properties: collections.OrderedDict[str, Union[int, str]] = collections.OrderedDict()
...
class FastaParser:
    def __init__(self, in_fasta_fn: Union[os.PathLike, str, bytes]):
    ...
    @staticmethod
    def load_fasta(in_fasta_fn: Union[os.PathLike, str, bytes]) -> dict[bytes, bytes]:
        chroms = {}
...
def map_path(module: str, name: str):
    ...
    print(f"module: {module}, mod: {str(mod)}")
...
class NewGeneIndex(NewGtfParser):
    def __init__(
        self,
        in_gtf_fn: Optional[Union[os.PathLike, str, bytes]] = None,
        in_fasta_fn: Optional[Union[os.PathLike, str, bytes]] = None,
    ):
        """Constructor.
    ...
    @staticmethod
    def load_from_reference(reference_path: Union[str, bytes]):
    ...
    @staticmethod
    def load_from_json(gene_index_fn: Union[str, bytes, os.PathLike]) -> NewGeneIndex:
        """Load the GeneIndex directly from the reference path.
        ...
        def conv_interval(obj: dict[str, Optional[Union[str, int]]]) -> Interval:
            if obj["strand"]:
                s = str(obj["strand"])
            else:
                s = None
    ...
    def gene_id_to_int(self, gene_id: bytes) -> Optional[int]:
    ...
    def get_gene(self, gene_id: bytes) -> Optional[Gene]:
        """Return the Gene object whose gene ID is gene_id or None otherwise."""
        assert isinstance(gene_id, bytes)
        index = self.gene_id_to_int(gene_id)
        return self.genes[index] if index < len(self.genes) else None
