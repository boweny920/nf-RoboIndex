...
class FeatureExtractor:
    ...
    def __init__(
        self, feature_ref: FeatureReference, use_feature_types: Optional[list[str]] = None
    ):
    ...
    @staticmethod
    def _compile(
        feature_defs: list[FeatureDef], use_feature_types: Optional[list[str]] = None
        ...
        for fd in feature_defs:
            ...
            if sequence in entry.barcode_dict:
                raise FeatureDefException(
                    'Found two feature definitions with the same read ("%s"), pattern ("%s") and barcode sequence ("%s"). This combination of values must be unique for each row in the feature definition file.'
    ...
    def _filter_feature_matches(
        matches: list[tuple[bytes, bytes, Optional[FeatureDef], int, None]],
...
def from_transcriptome_and_csv(
    gene_ref_path, feature_def_filename, target_features: Optional[dict[str, Iterable[int]]] = None
) -> FeatureReference:
...
def check_crispr_target_gene(
    ...
    target_features: Optional[dict[str, Iterable[int]]] = None,
):
...
def compile_pattern(pattern_str, length: int):
    ...
    try:
        ...
        raise FeatureDefException(
            'Failed to compile the feature definition string "%s" into a regular expression: %s.'
            % (regex_str, str(re.error))
        )
...
def parse_feature_def_file(filename, index_offset: int = 0) -> tuple[list[FeatureDef], list[str]]:
    ...
    for (row_num, row) in enumerate(feature_reader(reader)):
        ...
        if FeatureExtractor.get_feature_type(row) not in ALLOWED_FEATURE_TYPES:
            ...
            msg = (
                "Unknown feature_type: '%s'."
                "\nThe 'feature_type' field in the feature reference"
                " must be one of %s"
            ) % (FeatureExtractor.get_feature_type(row), options)
        ...
        if tags["read"] not in ALLOWED_READS:
            raise FeatureDefException(
                'The feature definition file contains a read type value "%s" which is not one of the allowed read types %s.'
