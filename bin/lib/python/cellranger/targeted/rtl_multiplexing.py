...
def _get_barcode_length(
    barcode_def: list[dict[str, Union[str, int]]], barcode_kind: str
) -> Optional[int]:
...
def get_barcode_locations(
    barcode_def: list[dict[str, Union[str, int]]]
) -> tuple[
...
def get_probe_bc_whitelist(barcode_def: list[dict[str, dict[str, str]]]) -> str:
    """Returns the whitelist name of the probe barcodes.

    Args:
        barcode_def: list of barcode definitions from chemistry def

    Returns:
        whitelist name of the probe barcodes
    """
    return next(
        barcode["whitelist"]["name"]
        for barcode in barcode_def
        if barcode["kind"] in ("left_probe", "right_probe")
    )
