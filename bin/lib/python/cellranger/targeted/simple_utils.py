...
def parse_target_csv(
    ...
    ref_gene_index: Optional[Union[GeneIndex, NewGeneIndex]] = None,
    gene_name_to_id: Optional[dict[bytes, bytes]] = None,
    filter_probes: bool = True,
    expected_targeting_method: Optional[str] = None,
...
def check_target_csv_metadata(
    ...
    if float(method_info.file_version) > 2.0:
        raise cr_csv_utils.CSVParseException(
            'The %s file %s contains an unknown %s: "%s". Must be 2.0 or less.'
            % (descriptive_name, filename, method_info.file_format_tag, method_info.file_version)
        )
...
def sanitize_gene_entry(
    raw_entry: bytes, ref_gene_index: GeneIndex, gene_name_to_id: dict[bytes, bytes]
) -> bytes:
    ...
    if raw_entry.split(b".")[0] in ref_gene_index.transcripts:
        gene_id = ref_gene_index.get_gene_from_transcript(raw_entry.split(b".")[0]).id
        return gene_id
    else:
