...
from cellranger.webshim.jibes_web import _make_rc_vectors, make_color_map, make_histogram_plot
            ...
            if prefix is not None:
                ...
                if "prefix_filter" in metric_dict:
                    if prefixes is not None:
                        ...
                        sys.stderr.write(
                            "Warning: no metric prefix values to filter for metric %s. The prefix name %s is probably wrong.\n"
                            % (name, prefix)
                        )
...
def plot_basic_barcode_rank(
    chart, cell_barcodes, barcode_summary, genomes, lib_prefix, restrict_barcodes=None
):
...
def plot_umi_depth(
    chart: dict,
    sample_properties: Union[dict, CountSampleProperties],
