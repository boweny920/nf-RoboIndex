...
def make_histogram_help(is_antibody):
    """Returns help for antibody/antigen umi count histogram"""
    if is_antibody:
        return {
            "helpText": f"Only Antibodies with total UMI counts over {MIN_COUNTS_PER_ANTIBODY:,} are plotted, and only up to the top {DEFAULT_MAX_HISTOGRAMS} Antibodies by UMI count are shown. The X-axis is the UMI counts in the log scale, while the Y-axis is the number of cells.",
            "title": "Histogram of Antibody Counts",
        }
    else:
        return {
            "helpText": "Only Antigens with non-zero total UMI counts are plotted. The X-axis is the UMI counts in the log scale, while the Y-axis is the number of cells.",
...
def make_histogram_as_bar_plot(
    ...
    if len(counts) > 0:
        ...
        if cutoff:
            totals = np.fromiter((x.total for x in bin_info), count=len(bin_info), dtype=np.float)
...
def make_antibody_histograms(ab_matrix):
    ...
    histogram_plot = make_histogram_as_bar_plot(
        data, color_map, "Log 10 (1 + UMI Count)", "Number of Cells"
    )
    PLOT_CONFIG[MODE_BAR_BUTTONS] = [["toImage"]]
    histogram_plot["config"] = PLOT_CONFIG
    histogram_data = {
        "plot": histogram_plot,
        "help": make_histogram_help(is_antibody),
    }
