...
def _sanitize_scalar(
    data: Union[bytes, float, np.integer, np.floating]
...
) -> Optional[Iterable[tuple[Any, Any]]]:
...
def json_sanitize(data) -> Union[None, dict[str, Any], int, str, float, list[Any]]:
    ...
    if isinstance(data, (bytes, float, np.floating, np.integer)):
        ...
        # Numpy 0-d array
        return _sanitize_scalar(np.asscalar(data))
    else:
        return data
...
def _make_iterencode(
    ...
    _sanitize_scalar: Callable[[Any], Union[None, str, int, float]] = _sanitize_scalar,
    _dict_items: Callable[[Any], Optional[Iterable[tuple[Any, Any]]]] = _dict_items,
    ...
    def _iterencode(obj: Any, depth: int) -> Generator[str, None, None]:
        if obj is None or (_is_safe(obj) if not indent else isinstance(obj, (str, int, bool))):
            ...
            yield from enc(_sanitize_scalar(np.asscalar(obj)), depth)
...
def dump_numpy(
    data: Any, fp: IO[str], *, pretty: bool = False, **kwargs
