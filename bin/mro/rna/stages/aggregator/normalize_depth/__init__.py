...
from cellranger.fast_utils import count_reads_per_library
    ...
    with MoleculeCounter.open(args.molecules, "r") as mc:
        ...
        if mc.feature_reference.has_target_features():
            ...
            try:
                ...
                target_feature_indices = sorted(mc.feature_reference.get_target_feature_indices())
                all_library_indices = [library["library_id"] for library in library_info]
                # For large AGGRs this loads all the barcodes and consumes a fair bit of memory
                usable_targeted_reads = count_reads_per_library(
                    args.molecules,
                    all_library_indices,
                    filter_feature_idx=target_feature_indices,
                    exclude_noncells=True,
                    custom_barcode_idx=set(),
                )
                for idx in gex_indices:
                    usable_reads[idx] = usable_targeted_reads[idx]
                reads_per_library = martian.make_path("reads_per_library.json")
                with open(reads_per_library, "w") as usable_reads_file:
                    usable_reads_file.write(json.dumps(usable_reads))
...
def _make_metrics_out(args, library_info: list[dict[str, Any]], mc: MoleculeCounter):
    ...
    # if is targeted aggr, get old targeted reads count
    if mc.feature_reference.has_target_features():
        gex_indices = mc.get_library_indices_by_type().get(
            rna_library.GENE_EXPRESSION_LIBRARY_TYPE, []
        )
        ...
        try:
            ...
            with open(args.reads_per_library) as usable_reads_file:
                usable_reads = json.loads(usable_reads_file.read())

        for (lib_idx, lib) in enumerate(library_info):
            if lib[rna_library.LIBRARY_TYPE] == rna_library.GENE_EXPRESSION_LIBRARY_TYPE:
                metrics_out[cr_mol_counter.LIBRARIES_METRIC][str(lib_idx)][
                    TARGETED_READ_PAIRS_METRIC
                ] = usable_reads[lib_idx]
            else:
                metrics_out[cr_mol_counter.LIBRARIES_METRIC][str(lib_idx)][
                    TARGETED_READ_PAIRS_METRIC
                ] = 0
    return metrics_out, frac_reads_kept, is_targeted_aggr
...
def _update_metrics(
    ...
    assert gem_groups[0] == 1 and gem_groups[-1] == len(gem_groups)
...
def main(args, outs):
    ...
    with MoleculeCounter.open(args.molecules, "r") as mc:
        ...
        metrics_out, frac_reads_kept, is_targeted_aggr = _make_metrics_out(args, library_info, mc)

        raw_umi_matrix, barcode_seqs, read_summary = _update_metrics(
            args, frac_reads_kept, mc, library_info, barcode_info, metrics_out
        )
    ...
    for lib_type_idx, lib_type in enumerate(lib_types):
        ...
        for lib_idx in lib_inds:
            ...
            summary.update(
                {
                    ...
                    "%s_%sfiltered_bcs" % p: n_cells,
                    "%s_%spre_normalization_feature_reads" % p: feature_reads,
                    ...
                    "%s_%spre_normalization_feature_reads_per_filtered_bc"
                    % p: pre_norm_feature_rppc,
                    "%s_%sintrons_included" % p: intron_mode_param,
                }
            )
            ...
            if TARGETED_READ_PAIRS_METRIC in lib_metrics.keys():
                ...
                summary.update(
                    {
                        "%s_%spre_normalization_targeted_cmb_reads_per_filtered_bc"
                        % p: pre_norm_targeted_rpc,
                    }
                )
