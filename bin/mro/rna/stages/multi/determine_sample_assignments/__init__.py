...
def _get_barcodes_per_tag(config: multi_config.CrMultiGraph, args, outs):
    ...
    if config.is_multiplexed():
        ...
        # however, in case of RTL multiplexing will include cell and non-cell barcodes
        ...
        if config.get_cell_multiplexing_type() == multi_config.CellMultiplexingType.CMO:
...
def _get_forced_mutiplets(
    ...
    args,
    outs,
    barcodes_per_tag: Optional[CellsPerFeature],
):
    assignment_metrics = {}
...
def _get_singleplex_multiplets(
    config: multi_config.CrMultiGraph, args, barcodes_per_tag: Optional[CellsPerFeature]
):
    assignment_metrics = {}
    filtered_sample_barcodes = SampleBarcodes()
    sample_barcodes = SampleBarcodes()
    assert len(config.samples) == 1  # no muxing, multi config must have only one sample
    sample_id = config.samples[0].sample_id.encode()
...
def _get_multiplex_multiplets(
    config: multi_config.CrMultiGraph, args, barcodes_per_tag: CellsPerFeature
):
    assignment_metrics = {}
    ...
    filtered_sample_barcodes = SampleBarcodes()
    sample_barcodes = SampleBarcodes()
    seen_tags = set()  # set(string)

    for sample in config.samples:
        sample_id = sample.sample_id.encode()
        filtered_sample_barcodes[sample_id] = []
        sample_barcodes[sample_id] = []

        for fingerprint in sample.fingerprints:
            if fingerprint.gem_well != args.gem_well:
                continue

            if fingerprint.tag_name.encode() not in barcodes_per_tag:
                # Tags with no cells assigned don't have an entry in cells_per_tag
                metric_name = f"tag_{fingerprint.tag_name}_number_of_singlets"
                assignment_metrics[metric_name] = 0
                continue

            if fingerprint.tag_name in seen_tags:
                martian.throw(
                    "Tag {} is used in more than one sample in gem-well {}.".format(
                        fingerprint.tag_name, fingerprint.gem_well
                    )
                )

            seen_tags.add(fingerprint.tag_name)
            ...
            singlet_barcodes_for_tag = [
                bc
                for bc in barcodes_per_tag[fingerprint.tag_name.encode()]
            ...
            all_barcodes_for_tag = [
                bc for bc in barcodes_per_tag[fingerprint.tag_name.encode()] if bc not in multiplets
            ]
...
def _get_multiplets(
    config: multi_config.CrMultiGraph, args, outs, barcodes_per_tag: Optional[CellsPerFeature]
):
...
def main(args, outs):
    ...
    if force_sample_barcodes is None:
        outs.sample_barcodes_json = martian.make_path("sample_barcodes.json")
        sample_barcodes.save_to_file(outs.sample_barcodes_json)

        # In case of CMOs this file should be identical to the above sample_barcodes_json
        outs.sample_cell_barcodes_json = martian.make_path("sample_cell_barcodes.json")
        filtered_sample_barcodes.save_to_file(outs.sample_cell_barcodes_json)
        ...
        if (
            config.is_multiplexed()
            and config.get_cell_multiplexing_type() == multi_config.CellMultiplexingType.RTL
        ):
